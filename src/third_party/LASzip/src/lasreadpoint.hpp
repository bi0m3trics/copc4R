...
public:
  ...
  // COPC extension: prepare for decompressing a single chunk without
  // the 8-byte chunk-table-offset prefix found in standard LAZ files.
  // Call after setup() + init() and before the first read().
  void prepare_single_chunk();
