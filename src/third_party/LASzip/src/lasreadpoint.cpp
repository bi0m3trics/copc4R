...
void LASreadPoint::prepare_single_chunk()
{
  // For COPC: the stream points directly at a single LAZ chunk's
  // compressed data (raw seed point + compressed payload).  We set
  // number_chunks to 1 so that init_dec() skips read_chunk_table().
  number_chunks = 1;
  tabled_chunks = 0;
  if (chunk_starts) free(chunk_starts);
  chunk_starts = (I64*)malloc_las(sizeof(I64) * 2);
  if (chunk_starts) { chunk_starts[0] = 0; chunk_starts[1] = 0; }
  current_chunk = 0;
}
